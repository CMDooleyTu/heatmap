...
regionCount     <- ifelse(is.na(Args[10]), 500,             as.integer(Args[10]))
...
names(data)[names(data) == 'Chromosome.scaffold.name']     <- 'Chr'
...
# Get counts
countData <- data[,#grepl(" counts$", names(data)) &
                  !grepl(" norm.count$", names(data))]
